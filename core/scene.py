"""
Specific file for handle what happen in each scene/current scene
"""
import pygame
...
class Scene:
    ...
    def __init__(self, zone_id, items):
        ...
        # load scene
        if self.zone_id != 7:
            self.bg_past = self.load_image(
                f"assets/image/background/zone{zone_id}_past.png")
            self.bg_present = self.load_image(
                f"assets/image/background/zone{zone_id}_present.png")
            self.bg_future = self.load_image(
                f"assets/image/background/zone{zone_id}_future.png")
        else:
            self.bg_past = self.load_image("assets/image/background/zone7.png")
            self.bg_present = self.load_image(
                "assets/image/background/zone7.png")
            self.bg_future = self.load_image(
                "assets/image/background/zone7.png")
    ...
    def draw(self, screen, current_time, selected_item, game):
        """
        draw object and background on screen
        """
    ...
    def check_hover(self, mouse_pos, current_time):
        """
        check whether players mouse is in object area that can click or not
        """
        for item in self.items:
            if item.is_active and item.rect.collidepoint(mouse_pos) and item.time_period == current_time:
                pos_in_mask = (mouse_pos[0] - item.rect.x,
                               mouse_pos[1] - item.rect.y)
    ...
    def handle_click(self, mouse_pos, game, current_time):
        """
        handle logic when click happen in scene
        """
        for item in self.items:
